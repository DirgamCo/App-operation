$(document).ready(function () {
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                if ($('#purchase_list_filter_city_id').length) {
                    d.city_id = $('#purchase_list_filter_city_id').val();
                }
                if ($('#purchase_list_filter_gen_location_id').length) { // Using 'gen_location_id' as per your Blade view
                    d.gen_location_id = $('#purchase_list_filter_gen_location_id').val();
                }
    ...
    $('#purchase_list_filter_gen_location_id, #purchase_list_filter_city_id').on('change', function () {
    ...
    if ($('#purchase_requisition_ids').length) {
        if (location_id !== '') {
            $.ajax({
                ...
                dataType: 'json',
                success: function (data) {
                    $('#purchase_requisition_ids').select2('destroy').empty().select2({
                        data: data
                    });
                    $('#purchase_entry_table tbody').find('tr').each(function () {
