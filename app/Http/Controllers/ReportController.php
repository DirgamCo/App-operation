...
class ReportController extends Controller
{
 ...
 /**    -------------------------------------------------------------------------- */
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type',
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state', 'loc_id','city_id']);
            ...
            $datatable = Datatables::of($products)
                ...
                ->addColumn('loc_name',function($row){
                    return $row->loc_name;
                })
        ...
        $units = Unit::where('business_id', $business_id)
                            ->pluck('short_name', 'id');
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        $locations = Location::select('id','name')->pluck('name','id'); 
        $cities = City::select('id','name')->pluck('name','id');

        return view('report.stock_report',compact('cities','locations'))
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->join('locations as loc','BL.location_id_id','=','loc.id')
                ->join('cities as CT','CT.id','=','loc.city_id')
                ->join('product_variations as pv', 'v.product_variation_id', '=', 'pv.id')
                ->join('contacts as c', 't.contact_id', '=', 'c.id')
                ->join('products as p', 'pv.product_id', '=', 'p.id')
            ...
            if (! empty($city_id)) {
                $query->where('CT.id', $city_id);
            }
            $loc_id = $request->get('loc_id', null);
            if (! empty($loc_id)) {
                $query->where('loc.id', $loc_id);
            }
        ...
        return view('report.product_sell_report',compact('cities','locations'))
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                ...
                ->join('locations as LOC','BL.location_id_id','=','LOC.id')
                ->join('cities as CT','LOC.city_id','=','CT.id')
            ...
            if (! empty($request->get('city_id'))) {
                $query->where('CT.id', $request->get('city_id'));
            }
            if (! empty($request->get('loc_id'))) {
                $query->where('LOC.id', $request->get('loc_id'));
            }
        ...
        return view('report.sell_payment_report',compact('cities','locations'))
    ...
    public function itemsReport()
    {
        ...
        if (request()->ajax()) {
            $query = TransactionSellLinesPurchaseLines::leftJoin('transaction_sell_lines 
                ...
                ->join('locations as loc','loc.id','=','bl.location_id_id')
                ->join('cities as ct','ct.id','=','loc.city_id')
                ->join(
                    'variations as v',
                    'PL.variation_id',
            ...
            return Datatables::of($query)
                ...
                ->addColumn('city_name',fn($row)=>$row->city_name)
                ->addColumn('loc_name',fn($row)=>$row->loc_name)
        ...
        return view('report.items_report',compact('cities','locations'))
        ->with(compact('suppliers', 'customers', 'business_locations'));
    }
    ...
    public function getStockValue()
    {
        ...
        $location_id = request()->input('location_id');
        $filters = request()->only(['category_id', 'sub_category_id', 'brand_id', 'unit_id','loc_id','city_id']);
