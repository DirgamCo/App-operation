...
class Contact extends Authenticatable
{
    ...
    public static function customersDropdown($business_id, $prepend_none = true, $append_id = true)
    {
        ...
        if ($append_id) {
            $all_contacts->select(
                ...
                'contacts.id',
                'contacts.mobile as phone'
                );
        } else {
            $all_contacts->select('contacts.id', DB::raw('contacts.name as customer'), 'contacts.mobile as phone');
        }
        ...
        $customers = $all_contacts->get()->mapWithKeys(function ($item) {
            return [$item->id => [
        ...
        //Prepend none
        if ($prepend_none) {
            $customers = $customers->prepend(['customer' => __('lang_v1.none'), 'phone' => ''], '');
        }
    ...
    public static function searchByPhone($business_id, $phone)
    {
        $all_contacts = Contact::where('contacts.business_id', $business_id)
                        ->whereIn('contacts.type', ['customer', 'both'])
                        ->where('contacts.mobile', 'like', '%' . $phone . '%')
                        ->active()
                        ->select(
                            'contacts.id',
                            DB::raw("IF(contacts.contact_id IS NULL OR contacts.contact_id='', CONCAT( COALESCE(contacts.supplier_business_name, ''), ' - ', contacts.name), CONCAT(COALESCE(contacts.supplier_business_name, ''), ' - ', name, ' (', contacts.contact_id, ')')) AS customer"),
        ...
        return $all_contacts->get()->mapWithKeys(function ($item) {
            return [$item->id => [
                'customer' => $item->customer,
                'phone' => $item->phone
            ]];
