...
@section('content')
    ...
    <section class="content no-print">
        @component('components.filters', ['title' => __('report.filters')])
            ...
            @if (empty($only) || in_array('sell_list_filter_customer_id', $only))
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input id="sell_list_filter_customer_id" name="customer_id" value="{{ request('customer_id') }}"
                            placeholder="Enter phone..." class="form-control" style="width: 100%;" />

                        <script>
                            const customersList = {!! collect($customers)->map(function ($item, $key) {
                                    return [
                                        'value' => (string) $key, // this is the ID that will be submitted
                                        'phone' => $item['phone'],
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                const input = document.querySelector('#sell_list_filter_customer_id');

                                const tagify = new Tagify(input, {
                                    whitelist: customersList,
                                    maxTags: 1,
                                ...
                                <div class='tagify__dropdown__item'>
                                    <strong>${tagData.phone}</strong> - ${tagData.customer}
                                </div>`
                                    }
                                });

                                tagify.on('add', function(e) {
                                    // Submit the form when a customer is selected
            ...
            <div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('city_id', __('City') . ':') !!} {{-- Assuming 'city' is a translatable string --}}
            ...
            {{-- New Location ID Filter (from locations table) --}}
            <div class="col-md-3">
                <div class="form-group">
                    {!! Form::label('loc_id', __('Location Inside City') . ':') !!} {{-- Choose an appropriate label --}}
                    {!! Form::select('loc_id', $locations, null, [
                        'class' => 'form-control select2',
                        'style' => 'width:100%',
                        'placeholder' => __('lang_v1.all'),
                    ]) !!}
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change',
                '#loc_id,#city_id,#sell_list_filter_location_id, #sell_list_filter_customer_id, #sell_list_filter_payment_status, #created_by, #sales_cmsn_agnt, #service_staffs, #shipping_status, #sell_list_filter_source, #payment_method',
                function() {
                    sell_table.ajax.reload();
                });
